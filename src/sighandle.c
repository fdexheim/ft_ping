...
static void			print_packets_info()
{
	...
	packet_ratio = 100 - 100 * ((float)(g_env->run_data.nb_packets_received)
		/ (float)(g_env->run_data.nb_packets_sent));
	printf("\n--- %s ping statistics ---\n", g_env->dest);
...
void				recap(int sig)
{
	...
	if (g_env->run_data.recorded_sucessful_iters > 0)
	{
		...
		mdev = bootleg_sqrt((g_env->run_data.sum2
			/ g_env->run_data.recorded_sucessful_iters) - (avg * avg));
